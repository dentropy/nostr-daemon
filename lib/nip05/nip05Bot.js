...
export async function nip05bot(args, options) {
    ...
    for (const domain_name_index in config.domain_names) {
        if (config.domain_names[domain_name_index].update_method == "rsync") {
            if (!("REMOTE_USER" in config.domain_names[domain_name_index].config)) {
                console.log(`Invalid Config, domain_name=${config.domain_names[domain_name_index].domain_name} method is missing REMOTE_USER`);
                process.exit()
            }
            if (!("REMOTE_HOST" in config.domain_names[domain_name_index].config)) {
                console.log(`Invalid Config, domain_name=${config.domain_names[domain_name_index].domain_name} method is missing REMOTE_HOST`);
                process.exit()
            }
            if (!("REMOTE_PATH" in config.domain_names[domain_name_index].config)) {
                console.log(`Invalid Config, domain_name=${config.domain_names[domain_name_index].domain_name} method is missing HOST_PATH`);
                process.exit()
            }
            if (!("REMOTE_PORT" in config.domain_names[domain_name_index].config)) {
                console.log(`Invalid Config, domain_name=${config.domain_names[domain_name_index].domain_name} method is missing REMOTE_PORT`);
                process.exit()
            }
        }
        if (config.domain_names[domain_name_index].update_method == "filepath") {
            if (!("NOSTR_JSON_PATH" in config.domain_names[domain_name_index].config)) {
                console.log(`Invalid Config, domain_name=${config.domain_names[domain_name_index].domain_name} method is missing NOSTR_JSON_PATH`);
                process.exit()
            }
        }
    }
    ...
    sub.on("event", async (event) => {
        ...
        if (getPublicKey(nip19.decode(args.nsec).data) != event.pubkey) {
            ...
            let update_nostr_dot_json = await nip05CheckNostrJsonUpdate(slash_command_results, config, nostr_dot_json)
            if (update_nostr_dot_json.command == "update_nostr_dot_joson") {
                nostr_dot_json[update_nostr_dot_json.data.domain_name] = update_nostr_dot_json.data.nostr_dot_json
            }
