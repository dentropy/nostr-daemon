    ...
    sub.on("event", async (event) => {
        ...
        if (getPublicKey(nip19.decode(args.nsec).data) != event.pubkey) {
            ...
            if (update_nostr_dot_json.command == "update_nostr_dot_joson") {
                let the_config = ""
                for (domain_name_config of config.domain_names) {
                    if (domain_name_config.domain_name == command.data.domain_name) {
                        the_config = domain_name_config
                    }
                }
                if (the_config == "") {
