...
*/
import { nip05SlashCommand_parser, nip05_help_command } from "./nip05SlashCommandParser.js";
...
Deno.test(`slash-command-prase "/nip05 help"`, async () => {
  const example_convo = [
    {
      decrypted_content: "/nip05 help",
    },
  ];
  const parsed_command = await nip05SlashCommand_parser(
    example_convo,
  ...
  assertEquals(nip05_help_command, parsed_command);
});

Deno.test(`slash-command-prase "/nip05 set-relays test@test.local:8090 relays: ws://test.local ws://nip05.local"`, async () => {
  const example_convo = [
    {
      decrypted_content: "/nip05 set-relays test@test.local:8090 relays: ws://test.local ws://nip05.local",
      pubkey: "hexpubkey"
    },
  ];
  const parsed_command = await nip05SlashCommand_parser(
