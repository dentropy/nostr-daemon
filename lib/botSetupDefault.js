...
export async function botSetupDefault(args, config_json_schema) {
    ...
    let allRelays = config.relays
    if( Object.keys(config).includes("nip65_metadata_profile_relays") ) {
        function mergeUniqueStrings(list1, list2) {
            return [...new Set([...list1, ...list2])];
        }
        allRelays = mergeUniqueStrings(config.relays, config.nip65_metadata_profile_relays)
    }
